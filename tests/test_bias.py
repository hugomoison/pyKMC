...
class TestDirectionBias:
    ...
    def test_custom_threshold_rejects_small_projection(
        self, system_origin, ref_table_atom0
    ):
    ...
    def test_custom_threshold_accepts_large_projection(
        self, system_origin, ref_table_atom0
    ):
    ...
    def test_atom_not_in_atom_set_pass_unlisted_true(
        self, system_origin, ref_table_atom0
    ):
    ...
    def test_atom_not_in_atom_set_pass_unlisted_false(
        self, system_origin, ref_table_atom0
    ):
    ...
    def test_enabled_false_bypasses_filter(self, system_origin, ref_table_atom0):
        ...
        idx, delta_t, ktot = bias.select(
            rejection_free, l_k, active, system_origin, ref_table_atom0
        )
        assert idx == 0
...
class TestPointBias:
    ...
    def test_atom_not_in_atom_set_pass_unlisted_true(self, ref_table_atom0):
        ...
        system = make_system([[0.0, 0.0, 0.0]])
        bias = PointBias(
            target_point=[5.0, 0.0, 0.0], atom_indices=[99], pass_unlisted=True
        )
        event = make_event(atom_index=0, final_positions=[[-1.0, 0.0, 0.0]])
        assert bias.accept(event, system, ref_table_atom0) is True
    ...
    def test_select_returns_accepted_event(self, ref_table_atom0):
        ...
        events = [
            make_event(
                atom_index=0, final_positions=[[-1.0, 0.0, 0.0]], k=10.0
            ),  # away
            make_event(
                atom_index=1, final_positions=[[1.0, 0.0, 0.0]], k=1.0
            ),  # toward
        ]
        active = make_active_table(events)
        l_k = np.array([e["k"] for e in events])
...
class TestTopoBias:
    ...
    def test_accept_when_source_topology_absent(self):
        """When no atom carries topo_source, no atom is a source atom."""
        system = make_system([[0.0, 0.0, 0.0], [5.0, 0.0, 0.0]])
    ...
    def test_select_falls_back_when_source_topology_absent(self):
        ...
        active = make_active_table(events)
        l_k = np.array([e["k"] for e in events])
        idx, _, ktot = bias.select(rejection_free, l_k, active, system, ref, ae)
        assert idx == 0
        assert ktot > 0.0
    ...
    def test_select_returns_event_reducing_distance(self):
        ...
        l_k = np.array([e["k"] for e in events])
        idx, delta_t, ktot = bias.select(
            rejection_free, l_k, active, system, ref, atomic_environment=ae
        )
        assert idx == 1
...
class TestBoostMode:
    ...
    def test_pass_unlisted_true_with_atom_indices_raises_direction(self):
        ...
        with pytest.raises(ValueError, match="pass_unlisted=True"):
            DirectionBias(
                direction=[1, 0, 0], atom_indices=[0], mode="boost", pass_unlisted=True
            )
    ...
    def test_pass_unlisted_false_with_atom_indices_is_valid(self):
        ...
        bias = DirectionBias(
            direction=[1, 0, 0], atom_indices=[0], mode="boost", pass_unlisted=False
        )
...
class TestPassUnlisted:
    def test_direction_pass_unlisted_false_non_listed(
        self, system_origin, ref_table_atom0
    ):
    ...
    def test_direction_pass_unlisted_true_non_listed(
        self, system_origin, ref_table_atom0
    ):
