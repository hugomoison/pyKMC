...
class ControlParameters(BaseModel):
    ...
    topology_search_status_output: Optional[str] = Field(
        default="./topology_search_status.pickle",
        description="File path where each topology's persistent search-completeness status (has it been through its own dispatched search session yet) will be stored in pickle format.",
    )
    ...
    topology_search_status: Optional[str] = Field(
        default=None,
        description="Path to a topology search-completeness status file generated from a previous simulation.",
    )
    ...
    session_size: Optional[int] = Field(
        default=1, gt=0, description="Number of MPI ranks per session/engine."
    )
...
class AtomicEnvironmentParameters(BaseModel):
    ...
    def validate_coordination_threshold(self) -> "AtomicEnvironmentParameters":
    ...
    coloring_mode: Literal["grey", "full"] = Field(
        default="full",
    ...
    rnei_pairs: Optional[dict[tuple[str, str], float]] = Field(
        default=None,
        description="Optional per-species-pair override of `rnei` (in Angstrom), as "
        "comma- and/or newline-separated 'A-B:value' entries, e.g. "
        "'Si-Si:2.5, Si-Fe:2.8, Fe-Fe:2.9'. Pairs not listed fall back to `rnei`. "
        "Requires coloring_mode='full'. Alternative to the top-level "
        "[AtomicEnvironmentRnei] section; the two cannot be combined.",
    )

    @field_validator("rnei_pairs", mode="before")
    @classmethod
    def parse_rnei_pairs(cls, v):
        """Parse comma-/newline-separated 'A-B:value' entries into {(species_a, species_b): value}."""
        ...
        for item in re.split(r"[,\n]", v):
            ...
            if not item:
                continue
            pair_str, _, value_str = item.partition(":")
            species_a, _, species_b = pair_str.strip().partition("-")
            pairs[tuple(sorted((species_a.strip(), species_b.strip())))] = float(
                value_str.strip()
            )
        return pairs

    @model_validator(mode="after")
    def validate_rnei_pairs_requires_full_coloring(self) -> "AtomicEnvironmentParameters":
        """Ensure rnei_pairs is only used with species-resolved coloring."""
        if self.rnei_pairs is not None and self.coloring_mode != "full":
            raise ValueError(
                "rnei_pairs requires coloring_mode='full' (species-resolved "
                "environments); it has no meaning under 'grey'."
            )
        return self
...
class EventSearchParameters(BaseModel):
    ...
    nsearch: Optional[int] = Field(
        default=None,
        description="Number of event searches to perform per unique atomic environment. "
        "Required when adaptive_search is False; unused when True.",
    )
    adaptive_search: bool = Field(
        default=False,
        description="Replace the fixed nsearch budget with a count-weighted adaptive "
        "stopping rule (see adaptive_stopping_rule): stream searches through a fixed-size "
        "worker pool, tracking rediscovery of already-catalogued events, and stop once the "
        "chosen rule considers a topology converged. Whenever a worker frees up, it's "
        "handed to whichever still-open topology needs it most: a topology already "
        "confirming is prioritized up to its exact remaining confirmation need, and only "
        "once no topology still needs confirming does the pool share across the rest. If "
        "still unconverged at half of adaptive_max_searches, searching continues "
        "(escalated) up to adaptive_max_searches before finally giving up.",
    )
    adaptive_stopping_rule: Literal["q_sprt", "q_qmin", "q_se"] = Field(
        default="q_sprt",
        description="Which adaptive stopping rule to use (see pykmc.adaptive_search's "
        "module docstring for the full derivation of each). All three are built on a "
        "topology's own discovery frequency (how often each cataloged event has been "
        "rediscovered), not its rate constant. 'q_sprt': a formal Wald sequential test on "
        "the dry spell since the last new discovery -- the best-balanced choice, no extra "
        "hyperparameters, and the default. 'q_qmin': tracks the same discovery-frequency "
        "fraction as 'q_sprt' but with a confirmation window that lengthens dynamically "
        "based on the topology's own thinnest observed evidence -- the most accurate of "
        "the three, at the highest search cost. 'q_se': the same fraction confirmed via a "
        "statistical confidence bound instead of a waiting window -- the cheapest, at a "
        "real cost in per-topology reliability (see as-diagnostic/RESEARCH_CONTEXT.md for "
        "the empirical comparison behind this choice).",
    )
    adaptive_scope: Literal["all", "important"] = Field(
        default="all",
        description="Which discovered events the chosen adaptive_stopping_rule's own estimator "
        "is built from. 'all': every discovered event counts, as described above. "
        "'important': the estimator is restricted to the topology's own currently-important "
        "events -- the smallest leading group, ranked by rate descending, whose cumulative "
        "rate covers ControlParameters.refine_thr of the topology's own currently-discovered "
        "total (the same cumulative-rate-coverage threshold execute_refinements uses), "
        "recomputed fresh as new discoveries arrive. A topology can then converge without "
        "ever confirming its own negligible-rate long tail -- cheaper, at the cost of the "
        "'all' guarantee that every discovered event itself gets confirmed.",
    )
    adaptive_epsilon: float = Field(
        default=0.05,
        gt=0.0,
        description="Accepted-risk threshold for the chosen adaptive_stopping_rule: how "
        "much of a topology's discovery-frequency mass may still plausibly be undiscovered "
        "when the rule declares convergence. Smaller values demand more evidence "
        "(more searches) before stopping; the exact confirmation requirement this implies "
        "depends on which adaptive_stopping_rule is selected.",
    )
    adaptive_max_searches: int = Field(
        default=1000,
        gt=0,
        description="Per-topology search ceiling in adaptive mode. A still-unconverged "
        "topology is flagged 'escalated' once it crosses half of this value, and finally "
        "capped and stopped once it reaches it.",
    )
    ...
    @model_validator(mode="after")
    def validate_adaptive_or_nsearch(self) -> "EventSearchParameters":
        """Ensure nsearch is set in fixed mode."""
        if self.adaptive_search:
            if self.nsearch is not None:
                warnings.warn(
                    "eventsearch.nsearch is set but is ignored because adaptive_search is "
                    "True (searches per topology are instead governed by "
                    "adaptive_epsilon/adaptive_max_searches).",
                    stacklevel=2,
                )
        elif self.nsearch is None:
            raise ValueError(
                "nsearch is required when eventsearch.adaptive_search is False."
            )
        return self


class PartnParameters(BaseModel):
    """pARTn parameters."""
...
class RateConstantParameters(BaseModel):
...
class LammpsParameters(BaseModel):
    """Lammps parameters."""
...
class OTFMLParameters(BaseModel):
...
class BasinParameters(BaseModel):
    """Basin parameters"""
...
class RegionParameters(BaseModel):
    ...
    Used for ``inactive_atoms`` and ``frozen_atoms`` params sections.
    ...
    @model_validator(mode="after")
    def check_fields(self) -> "RegionParameters":
...
class BiasParameters(BaseModel):
...
class Parameters(BaseModel):
    """Parameters for the KMC simulations."""

    control: ControlParameters = Field(
        default_factory=ControlParameters, description="Core simulation control parameters."
    )

    atomicenvironment: AtomicEnvironmentParameters = Field(
        default_factory=AtomicEnvironmentParameters,
        description="Parameters defining the local atomic environments and the method used to define them.",
    )

    atomicenvironmentrnei: Optional[dict[str, float]] = Field(
        default=None,
        description="Optional [AtomicEnvironmentRnei] section: one 'Species-Species = value' "
        "line per pair, merged into atomicenvironment.rnei_pairs. Alternative to setting "
        "rnei_pairs inline; the two cannot be combined.",
    )

    eventsearch: EventSearchParameters = Field(
        default_factory=EventSearchParameters,
        description="Parameter controling the event searches.",
    )

    psr: PSRParameters = Field(
        default=PSRParameters,
        description="Parameter controlling the point set registration algorithm.",
    )
    rateconstant: RateConstantParameters = Field(
        default_factory=RateConstantParameters,
        description="Parameters used to compute rate constants.",
    )

    lammps: Optional[LammpsParameters] = Field(
        default=None,
        description="LAMMPS-specific parameters. Required if engine == lammps.",
    )
    otfml: Optional[OTFMLParameters] = Field(
        default=None,
        description="On-the-fly machine learning potential retraining parameters.",
    )

    partn: Optional[PartnParameters] = Field(
        default=None, description="pARTn parameters controling the event searches"
    )

    ira: Optional[IraParameters] = Field(default=None, description="IRA parameters.")

    basin: Optional[BasinParameters] = Field(default=None, description="Basin parameters")

    reconstruction: ReconstructionParameters = Field(
        default_factory=ReconstructionParameters, description="Reconstruction parameters"
    )
    ...
    eventrecycling: Optional[EventRecyclingParameters] = Field(
        default=None,
        description="Event recycling parameters. Required when control.recycle = True.",
    )

    inactive_atoms: Optional[RegionParameters] = Field(
        default=None,
    ...
    frozen_atoms: Optional[RegionParameters] = Field(
        default=None,
    ...
    bias: Optional[BiasParameters] = Field(
        default=None, description="Event selection bias parameters."
    )

    @classmethod
    def from_ini_file(cls, ini_path: str) -> Parameters:
        ...
        against the Pydantic `Parameters` model, providing clear error messages for
        ...
        -------
        Parameters
            A validated `Parameters` instance containing all simulation parameters.
    ...
    @model_validator(mode="after")
    def validate_dependencies(self) -> Parameters:
        ...
        -------
        Self
            The validated `Parameters` instance.
        ...
        if self.control.active_volume and self.activevolume.ract <= self.atomicenvironment.rcut:
    ...
    @model_validator(mode="after")
    def merge_atomicenvironmentrnei_section(self) -> Parameters:
        """Merge the optional [AtomicEnvironmentRnei] section into atomicenvironment.rnei_pairs."""
        if self.atomicenvironmentrnei is None:
            return self
        if self.atomicenvironment.rnei_pairs is not None:
            raise ValueError(
                "rnei_pairs is set both inline (atomicenvironment.rnei_pairs) and via "
                "the [AtomicEnvironmentRnei] section. Use only one."
            )
        if self.atomicenvironment.coloring_mode != "full":
            raise ValueError(
                "[AtomicEnvironmentRnei] requires coloring_mode='full' (species-resolved "
                "environments); it has no meaning under 'grey'."
            )
        self.atomicenvironment.rnei_pairs = {
            tuple(sorted(pair.split("-"))): value
            for pair, value in self.atomicenvironmentrnei.items()
        }
        return self
