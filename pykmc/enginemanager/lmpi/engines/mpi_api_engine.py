...
class MpiApiEngine:
    ...
    def start(self) -> None:
        ...
        if self.local_rank == 0 and isinstance(self.local_messenger, QueueMessenger):
            # engine_use_rank_0=True: this process is both the manager
            # (which constructs/issues session commands right after
            # `launch()` returns) and this chunk's own rank-0 engine worker.
            # `self.messenger`/`self.rank` are bound to the global mode at
            # this point (set by `use_global()` in __init__), and the
            # global messenger is always an MpiMessenger regardless of this
            # scenario -- so the check has to be against the local
            # messenger/rank instead, which `ManagerFactory` does make a
            # QueueMessenger exactly when this rank is in its own chunk.
            # Running the loop synchronously here would block this thread
            # forever probing for a message the manager side can never
            # send, since the code that would send it never gets to run on
            # this same thread; a background thread frees this one to
            # return and let the manager/session side proceed.
