...
class MpiApiSession:
    ...
    # @session_locked
    def initialize_system(self, configuration, params=None) -> None:
        """
        ...
        self.send_message(
            {
                ...
                "value": {"configuration": configuration, "params": params},
            }
        )

    # @session_locked
    def initialize_potential(self, params) -> None:
        """
        Initialize Lammps potential
        """
        # print(f"[Session {self.session_id}] Initializing Lammps Potential")
        self.send_message({"type": "initialize_potential", "value": params})

    def setup_otf_cycle(self, params) -> None:
        """Reload an updated LAMMPS potential in-place."""
        self.send_message({"type": "setup_otf_cycle", "value": params})
    ...
    # @session_locked
    def minimize(self, params, positions=None) -> None:
        """
        Minimize the system
        """
        # print(f"[Session] Minimizing the system")
        self.send_message(
            {"type": "minimize", "value": {"params": params, "positions": positions}}
        )
            ...
            self.send_message(
                {
                    ...
                    "value": {"params": params, "configuration": configuration},
                }
            )
            msg = self.messenger.recv(source=self.engine_master_rank, tag=1)
            if msg.get("type") == "result":
    ...
    # @session_locked
    def partn_search(self, params, central_atom_idx, configuration):
        self._is_busy = True
