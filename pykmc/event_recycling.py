...
class Recycling(ABC):
    ...
    together with system configuration snapshots taken before and after the
    ...
    def select_recyclable(
        ...
        configuration_pre : Configuration
            Snapshot of `system.configuration` taken just before the event was
            applied (same atom ordering as `system.configuration`).
...
class DistanceRecycling(Recycling):
    ...
    def select_recyclable(
        ...
        disp = per_atom_displacement(configuration_pre, system.configuration)
