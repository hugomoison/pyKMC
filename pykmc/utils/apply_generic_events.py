...
def _apply_event_generic(self, idx_atom_apply_event, idx_event_table):
    """ """
    if self.params.control.reconstruction:
        ...
        if rmat is None or dh > self.params.psr.matching_score_thr:
            return False
        else:
            ...
            dE = Esad - Eini
            if (
                abs(dE - self.reference_table.table.loc[idx_event_table]["dE_forward"])
                < 0.5
            ):
                new_positions = np.zeros(
                    (
